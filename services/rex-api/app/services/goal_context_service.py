...
from app.services.accountability_snapshot import active_plans_for
...
GOAL_PROGRESS_PATTERNS = (
    ...
    re.compile(
        r"\bwhat\s+(?:goals?|plans?)\s+do\s+(?:we|i)\s+have\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:list|show|tell me)\s+(?:me\s+)?(?:my|our)\s+"
        r"(?:saved\s+)?(?:goals?|plans?)\b",
        re.IGNORECASE,
    ),
)
...
class GoalContextService:
    ...
    def merge_structured_context(self, base: dict, goal_context: dict) -> dict:
        ...
        merged = {**base}
        for key in ("plans", "plan_milestones"):
