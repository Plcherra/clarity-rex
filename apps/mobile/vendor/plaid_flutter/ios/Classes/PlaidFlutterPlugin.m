...
- (void) createWithArguments: (id _Nullable)arguments withResult:(FlutterResult)result {
    ...
    PLKOnEventHandler eventHandler = ^(PLKLinkEvent *event) {
        ...
        // Keep the Link handler alive after HANDOFF so iOS OAuth redirects can
        // resume the same Link session and emit the final onSuccess callback.
...
- (void) resumeAfterTermination: (id _Nullable)arguments withResult:(FlutterResult)result{
    ...
    if (!redirectUriURL) {
        result([FlutterError errorWithCode:@"INVALID_REDIRECT_URI"
                                   message:@"Plaid OAuth redirect URI is invalid."
                                   details:nil]);
        return;
    }

    if (!_linkHandler) {
        result([FlutterError errorWithCode:@"PLAID_LINK_HANDLER_MISSING"
                                   message:@"Plaid Link handler was released before OAuth resume."
                                   details:nil]);
        return;
    }
